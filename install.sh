...
files=(
  "./packages/linux-charcoal-611-headers-6.11.11.valve26-5-x86_64.pkg.tar.zst"
  "./packages/linux-charcoal-611-6.11.11.valve26-5-x86_64.pkg.tar.zst"
...
checksums=(
  "0efee9f8b8b90855946ee7f212c684c9989dc03e214c693f4640942b22f7150a"
  "02d406b35ad58e5504d8b25e7b11a567a5267bd47e35a993c04a626c2a1fb487"
...
sdkernel() {
  while true; do
    ...
    if [[ "$answer" == "y" || "$answer" == "Y" ]]; then
      ...
      sudo pacman -U --noconfirm ./packages/linux-charcoal-611-6.11.11.valve26-5-x86_64.pkg.tar.zst >>"$LOG_FILE" 2>&1
      sudo pacman -R --noconfirm linux-neptune-611-headers >>"$LOG_FILE" 2>&1
      sudo pacman -U --noconfirm ./packages/linux-charcoal-611-headers-6.11.11.valve26-5-x86_64.pkg.tar.zst >>"$LOG_FILE" 2>&1
